...
#| echo: false
#| results: 'asis'
dinner.recipes <- dinner[sample(x=1:length(dinner), size=3, replace = TRUE)]
breakfast.recipe <- breakfasts[sample(x=1:length(breakfasts), size=1, replace = TRUE)]
recipe <- c(breakfast.recipe, dinner.recipes)
recipe <- unlist(recipe, recursive = FALSE)
...
ingredients <- html_elements(ingredients.section, "li")
ormatted_list <- paste("-", html_text(ingredients))
cat(formatted_list, sep = "\n")
}
ingredients(2)
```
